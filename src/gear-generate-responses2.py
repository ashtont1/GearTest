...
cache_dir = "/dataheart/ashton/cache"
...
#truncate last token of kv, and just use input_ids1 in second model.generate call
    ...
    pos = gen_text.rindex("ASSISTANT:")
    remaining_text = gen_text[pos + len("ASSISTANT:"):]
    with open(f"{args.results_dir}/{i}_result.txt", "w") as f:
        f.write(remaining_text)
    print("remaining_text after second generate on ", i, ".txt: \n", remaining_text)
    print('-'*50)
